...
export default function SelectLabel() {
  ...
  useEffect(() => {
    // Function to get the parent stack's width
    const getParentWidth = () => {
      if (containerRef.current) {
        // Get the closest parent with class 'MuiStack-root'
        const parentStack = containerRef.current.closest('.MuiStack-root');
        if (parentStack) {
          setContainerWidth(parentStack.offsetWidth);
        }
      }
    };
  ...
  return (
    <Box ref={containerRef} width="100%">
      ...
      <Popover
        ...
        sx={{
          '& .MuiPopover-paper': {
            width: containerWidth || anchorEl?.clientWidth || 'auto',
